...
describe('dynamic launch service', () => {
  ...
  it('maps dynamic market-cap/proceeds config into SDK builder with uniswapV4 migration', async () => {
    const builder = {
      tokenConfig: vi.fn().mockReturnThis(),
      saleConfig: vi.fn().mockReturnThis(),
      withMarketCapRange: vi.fn().mockReturnThis(),
    ...
    expect(builder.withMigration).toHaveBeenCalledWith({
      type: 'uniswapV4',
      fee: 20_000,
      tickSpacing: 100,
      streamableFees: {
  ...
  it('rejects dynamic launches that do not use uniswapV2 or uniswapV4 migration', async () => {
    const sdk = {
    ...
    ).rejects.toThrow(/dynamic launches require migration\.type="uniswapV2" or "uniswapV4"/i);
  });
});
