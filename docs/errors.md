...
`details` is optional. Zod validation failures return `422 INVALID_REQUEST` with
structured validation details.

For `5xx` responses, the API preserves the error code but replaces the message
with `"Internal server error"` and omits `details`.
...
EVM request schemas reject unknown properties and unsupported migration shapes as
`422 INVALID_REQUEST`. `poolFeeBeneficiaries` cannot be used with
`migration.type="uniswapV2"`; that migration accepts its singular
`feeBeneficiary` with an integer percentage from 1 through 50. Migration is
accepted only for dynamic launches.
...
Solana `feeBeneficiaries` uses `shareBps` and is independent of the EVM
beneficiary fields.
...
- `GET /ready` replaces dependency failure details with
  `"dependency unavailable"`.
- Shared deployments require Redis-backed idempotency, Redis-backed rate
  limiting, reachable Redis at startup, and idempotency keys on create requests.
